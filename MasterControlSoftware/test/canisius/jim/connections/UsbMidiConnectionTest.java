...
class UsbMidiConnectionTest extends MidiDeviceConnectionTest {
    ...
    @Test
    void doesUSBMidiDeviceExist() throws Exception {
        // Whether the USB MIDI device exists on the system or not, the Callable's result should match the result of the
        // method.
        final Callable<Boolean> UsbMidiDeviceChecker = () ->
                Arrays.stream(MidiSystem.getMidiDeviceInfo()).map(MidiDevice.Info::getName).anyMatch(name -> name.contains("USB") && name.contains("MIDIOUT"));
    ...
    void getUsbReceiver() {
        ...
        UsbMidiConnection.getInstance().connect();
        assertTrue(UsbMidiConnection.getInstance().getMidiDevice().getReceivers().contains(UsbMidiConnection.getInstance().getUsbReceiver()));
    }
}